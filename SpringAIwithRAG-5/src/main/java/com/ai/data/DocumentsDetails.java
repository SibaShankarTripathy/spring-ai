...
public class DocumentsDetails {
	...
	private static final Logger logger = LoggerFactory.getLogger(DocumentsDetails.class);
	...
	public String saveDocument() {
		...
		try {
			...
			TextReader textReader = new TextReader(new ClassPathResource("product-data.txt"));
...
//			List<Document> splittedTextDocument = textSplitter.split(textReader.get());
			...
			// Internally .add() -> doAdd() method will embedded the chunk and store.
			vectorStore.add(splittedTextDocument);
			response = "Document details insertion completed.....";
...
/*
 ** Equivalent Requests
PUT /collections/myProductDetails 
{
  "vectors": {
    "": {
      "size": 1024,
